...
{
    public class TcpSynScanner : IPortScanner
    {
        public bool RequiresElevatedPrivileges => true;
        public string ScannerName => "TCP SYN Scan (-sS)";
        ...
        public async Task<PortScanResult> ScanPortAsync(string ipAddress, int port, int timeout, CancellationToken cancellationToken)
        {
            ...
            try
            {
                using (var socket = new Socket(AddressFamily.InterNetwork, SocketType.Raw, ProtocolType.IP))
                {
                    ...
                    byte[] packet = CreateSynPacket(ipAddress, port);
            ...
            result.IsOpen = result.Status == "Open";
            return result;
        }

        private byte[] CreateSynPacket(string destIp, int destPort)
        {
            ...
            // SYN flag
            packet[IP_HEADER_LENGTH + 13] = 0x02;
        ...
        private string AnalyzeResponse(byte[] buffer, int bytesReceived)
        {
            ...
            bool ackSet = (flags & 0x10) != 0;
            bool synSet = (flags & 0x02) != 0;
